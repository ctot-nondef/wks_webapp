    ...
    >
      <template #body="{ items }">
        <tbody>
          ...
          >
            <td
              v-if="item.data.creators[0].creatorType!='author'"
              class="text-xs-right"
            >
              <span
                v-for="(author, index) in item.data.creators"
                :key="index"
              >{{ author.firstName }} {{ author.lastName }} [ {{ author.creatorType }} ]/</span>
            </td>
            <td
              v-if="item.data.creators[0].creatorType=='author'"
              class="text-xs-right"
            >
              <span>{{ item.data.creators[0].firstName }} {{ item.data.creators[0].lastName }} [ {{ item.data.creators[0].creatorType }} ]/</span>
            </td>
